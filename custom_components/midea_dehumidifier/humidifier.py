...
DEHUMIDIFIER_MODES_DICT = {'TARGET_HUMIDITY': 1, 'CONTINUOUS': 2, 'SMART': 3, 'DRYER': 4}
DEHUMIDIFIER_MODES_LIST = ['Target_humidity', 'Continuous', 'Smart', 'Dryer']

DEHUMIDIFIER_FAN_SPEED_DICT = {'SILENT': 40, 'MEDIUM': 60, 'HIGH': 80}
DEHUMIDIFIER_FAN_SPEED_LIST = ['Silent', 'Medium', 'High']
...
async def async_setup_platform(hass, async_add_entities, discovery_info=None):
    ...
    _LOGGER.debug(LOG_TAG + ": MIDEA_API_CLIENT=" + MIDEA_API_CLIENT)
    _LOGGER.debug(LOG_TAG + ": MIDEA_TARGET_DEVICE=" + MIDEA_TARGET_DEVICE)
    ...
    if targetDevice is not None:
        ...
        _LOGGER.info(LOG_TAG + ": humidifier entity initialized.")
    else:
        _LOGGER.error(LOG_TAG + ": error initializing humidifier entity.")
    ...
    hass.services.async_register(DOMAIN, SERVICE_SET_FAN_SPEED, async_service_set_fan_speed,
                                 SERVICE_SET_FAN_SPEED_SCHEMA)
    hass.services.async_register(DOMAIN, SERVICE_SET_ION_STATE, async_service_set_ion_state,
...
class MideaDehumidifierDevice(HumidifierEntity):
    ...
    def __init__(self, hass, client, targetDevice):
        _LOGGER.debug(LOG_TAG + ": initializing MideaDehumidifierDevice...")

        self._hass = hass
        self._supported_features = SUPPORT_FLAGS

        # Fan modes
        self._fan_dict = DEHUMIDIFIER_FAN_SPEED_DICT
        self._fan_list = DEHUMIDIFIER_FAN_SPEED_LIST

        # Device modes
        self._modes_dict = DEHUMIDIFIER_MODES_DICT
        self._available_modes = DEHUMIDIFIER_MODES_LIST
        ...
        self._humidity_set = None  # target humidity
        ...
        # if res == 1:
        #    _LOGGER.debug("midea_dehumidifier: get_device_status succeeded: "+self._client.deviceStatus.toString())
    ...
    async def service_set_fan_speed(self, speed_mode):
        ...
        _LOGGER.info(LOG_TAG + ": service_set_fan_speed called, speed_mode = %s", speed_mode)
        speed = self._fan_dict.get(speed_mode.upper(), 0)
        _LOGGER.info(LOG_TAG + ": speed = %s", speed)
        if self.is_on and self._windSpeed != speed and self._client.deviceStatus.setMode != 4:
            _LOGGER.debug(LOG_TAG + ": sending send_fan_speed_command via Web API...")
            res = await self.hass.async_add_executor_job(self._client.send_fan_speed_command, self._device["id"], speed)
            if res is not None:
                _LOGGER.debug(
                    LOG_TAG + ": send_fan_speed_command succeeded: " + self._client.deviceStatus.toString())
                ...
                _LOGGER.error(LOG_TAG + ": send_fan_speed_command ERROR.")

    @callback
    async def service_set_ion_state(self, ion_state):
        """service_set_ion_state"""
        _LOGGER.info(LOG_TAG + ": service_set_ion_state called, ion_state = %s", ion_state)
        if self.is_on and self._ionSetSwitch != ion_state:
            if ion_state:
                _LOGGER.debug(LOG_TAG + ": sending send_ion_on_command via Web API...")
                res = await self.hass.async_add_executor_job(self._client.send_ion_on_command, self._device["id"])
            else:
                _LOGGER.debug(LOG_TAG + ": sending send_ion_off_command via Web API...")
                res = await self.hass.async_add_executor_job(self._client.send_ion_off_command, self._device["id"])
            if res is not None:
                _LOGGER.debug(
                    LOG_TAG + ": send_ion_(on/off)_command succeeded: " + self._client.deviceStatus.toString())
    ...
    async def service_set_mode(self, mode_name):
        """service_set_mode"""
        _LOGGER.info(LOG_TAG + ": service_set_mode called, mode_name = %s", mode_name)
        mode = self._modes_dict.get(mode_name.upper(), 0)
        _LOGGER.info(LOG_TAG + ": mode = %s", mode)
        if self.is_on and self._mode != mode:
            _LOGGER.debug(LOG_TAG + ": sending send_mode_command via Web API...")
            res = await self.hass.async_add_executor_job(self._client.send_mode_command, self._device["id"], mode)
            if res is not None:
                _LOGGER.debug(LOG_TAG + ": send_mode_command succeeded: " + self._client.deviceStatus.toString())
                ...
                _LOGGER.error(LOG_TAG + ": send_mode_command ERROR.")

    async def async_update(self):
        """Retrieve latest state from the appliance and keep UI updated with respect to the updated status."""
        _LOGGER.info(LOG_TAG + ": async_update called.")

        if self._client.security.access_token:
            _LOGGER.debug(LOG_TAG + ": sending get_device_status via Web API...")
            # res = self._client.get_device_status(self._device['id'])
            ...
            if res == 1:
                ...
                _LOGGER.error(LOG_TAG + ": get_device_status ERROR.")
    ...
    async def async_turn_on(self, **kwargs):
        """Turn the device on."""
        _LOGGER.info(LOG_TAG + ": async_turn_on called.")
        if not self.is_on:
            _LOGGER.debug(LOG_TAG + ": sending async_turn_on command via Web API...")
            res = await self.hass.async_add_executor_job(self._client.send_poweron_command, self._device["id"])
            if res is not None:
                _LOGGER.debug(
                    LOG_TAG + ": async_turn_on succeeded: " + self._client.deviceStatus.toString())
                # Refresh device status
                self.__refresh_device_status()
            else:
                _LOGGER.error(LOG_TAG + ": async_turn_on ERROR.")

    async def async_turn_off(self, **kwargs):
        """Turn the device off."""
        _LOGGER.info(LOG_TAG + ": async_turn_off called.")
        if self.is_on:
            _LOGGER.debug(LOG_TAG + ": sending power-off command via Web API...")
            res = await self.hass.async_add_executor_job(self._client.send_poweroff_command, self._device["id"])
            if res is not None:
                _LOGGER.debug(
                    LOG_TAG + ": send power-off command succeeded: " + self._client.deviceStatus.toString())
                # Refresh device status
                self.__refresh_device_status()
            else:
                _LOGGER.error(LOG_TAG + ": send power-off command ERROR.")

    async def async_set_humidity(self, humidity):
        """Set new humidity level."""
        _LOGGER.info(LOG_TAG + ": async_set_humidity called.")
        if self.is_on:
            if self._humidity_set != humidity:
                _LOGGER.debug(LOG_TAG + ": setting new target humidity value via Web API...")
                # res = self._client.send_target_humidity_command(self._device["id"], humidity)
                res = await self.hass.async_add_executor_job(self._client.send_target_humidity_command,
                                                             self._device["id"], humidity)
                if res is not None:
                    _LOGGER.info(
                        LOG_TAG + ": async_set_humidity succeeded: " + self._client.deviceStatus.toString())
                    # Refresh device status
                    self.__refresh_device_status()
                else:
                    _LOGGER.error(LOG_TAG + ": async_set_humidity ERROR")

    async def async_set_mode(self, mode):
        """Update mode."""
        _LOGGER.info(LOG_TAG + ": async_set_mode called; current_mode=%s, new mode=%s", self._mode, mode)
        if self.is_on:
            mode_num = self._modes_dict.get(mode.upper(), 0)
            _LOGGER.debug(LOG_TAG + ": sending update status command via Web API...")
            ...
            if res is not None:
                _LOGGER.debug(LOG_TAG + ": async_set_mode succeeded: " + self._client.deviceStatus.toString())
                self._client.deviceStatus._setMode = mode_num
                # Refresh device status
                self.__refresh_device_status()
            else:
                _LOGGER.error(LOG_TAG + ": async_set_mode ERROR.")
